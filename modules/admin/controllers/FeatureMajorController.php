 ...
 * @version 1.0
 */
class Admin_FeatureMajorController extends Zend_Controller_Action
{
    /**
     * 成员变量
     */
    ...
    {
        $this->modelFeatureMajor = new Admin_Model_FeatureMajor();
    ...
    public function manageAction()
    {
         ...
         * 获得数据
         */
        $data = $this->modelFeatureMajor->getList($where,$order);
        if ($data != null) {
            $this->view->data = $data;
    ...
    public function importAction()
    {
        ...
        if ($this->_request->isPost() && isset($_FILES["data-file-upload"])) {
            ...
            if ($dataFile['error'] == 0) {
                ...
                $isSuccess = $this->modelFeatureMajor->importExcel($tempFileName);
                if ($isSuccess == true) {
                    $this->view->messages[] = array(
                        "恭喜您，数据导入成功！", 'success');
                } else {
    ...
    public function exportAction()
    {
        ...
        header ( "Content-Disposition:filename=特色专业".date("YmdHis").".xls" );
    ...
    {
        // 获取总记录数量
        $this->view->totalRowsCount = $this->modelFeatureMajor->getCount();
        
        $modelBase = new Application_Model_Base();
        $tableName = 'zf_feature_major';
        // 根据标题统计
    ...
    {
        $form = new Application_Form_FeatureMajor();
        $extra = array();

        if ($this->getRequest()->isPost()) {
            if ($form->isValid($_POST)) {
                $newid = $this->modelFeatureMajor->insertByForm($form, $extra);
    ...
    public function editAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($_POST)) {
                $result = $this->modelFeatureMajor->updateByForm($form, $id);
                if ($result) {
        ...
        $oldData = $this->modelFeatureMajor->getSingle(array('id'=>$id));
    ...
    public function deleteAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            try {
                $id = $this->_request->getParam('id');
                $this->modelFeatureMajor->deleteData($id);
