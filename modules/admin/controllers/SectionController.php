...
class Admin_SectionController extends Zend_Controller_Action
{
    ...
    {
        $this->modelSection = new Admin_Model_Section();
    ...
    public function manageAction()
    {
         ...
         * 获得数据
         */
        $data = $this->modelSection->getList($where,$order);
        if ($data != null) {
            $this->view->data = $data;
    ...
    public function importAction()
    {
        ...
        if ($this->_request->isPost() && isset($_FILES["data-file-upload"])) {
            ...
            if ($dataFile['error'] == 0) {
                ...
                $isSuccess = $this->modelSection->importExcel($tempFileName);
                if ($isSuccess == true) {
                    $this->view->messages[] = array(
                        "恭喜您，数据导入成功！", 'success');
                } else {
    ...
    public function exportAction()
    {
        ...
        header ( "Content-Disposition:filename=重点科室".date("YmdHis").".xls" );
    ...
    {
        // 获取总记录数量
        $this->view->totalRowsCount = $this->modelSection->getCount();
        
        $modelBase = new Application_Model_Base();
        $tableName = 'zf_section';
        // 根据科室名称统计
        $sql = "SELECT section, count(*) as count FROM $tableName GROUP BY section ORDER BY count DESC";
        $this->view->dataSection = $modelBase->rawQuery($sql);
    ...
    public function addAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($_POST)) {
                $newid = $this->modelSection->insertByForm($form, $extra);
    ...
    public function editAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($_POST)) {
                $result = $this->modelSection->updateByForm($form, $id);
                if ($result) {
        ...
        $oldData = $this->modelSection->getSingle(array('id'=>$id));
    ...
    public function deleteAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            try {
                $id = $this->_request->getParam('id');
                $this->modelSection->deleteData($id);
    ...
    private function getWhereStr()
    {
        ...
        if (isset($section)) {
            $where .= " AND section LIKE '%{$section}%'";
            $this->view->search_value_section = $section;
        }
