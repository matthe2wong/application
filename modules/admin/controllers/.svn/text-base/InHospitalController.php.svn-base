 ...
 * @version 1.0
 */
class Admin_InHospitalController extends Zend_Controller_Action
{
    /**
     * 成员变量
     */
    ...
    {
        $this->modelInHospital = new Admin_Model_InHospital();
    ...
    public function manageAction()
    {
         ...
         * 获得数据
         */
        $data = $this->modelInHospital->getList($where,$order);
        if ($data != null) {
            $this->view->data = $data;
    ...
    public function importAction()
    {
        ...
        if ($this->_request->isPost() && isset($_FILES["data-file-upload"])) {
            ...
            if ($dataFile['error'] == 0) {
                ...
                $isSuccess = $this->modelInHospital->importExcel($tempFileName);
                if ($isSuccess == true) {
                    $this->view->messages[] = array(
                        "恭喜您，数据导入成功！", 'success');
                } else {
    ...
    public function exportAction()
    {
        ...
        header ( "Content-Disposition:filename=住院就诊人信息".date("YmdHis").".xls" );
    ...
    public function viewAction()
    {
        ...
        $this->view->data = $this->modelInHospital->getSingle(array("id"=>$id));
    }
    ...
    {
        // 获取总记录数量
        $this->view->totalRowsCount = $this->modelInHospital->getCount();
        
        $modelBase = new Application_Model_Base();
        $tableName = 'zf_in_hospital';
        // 根据用户ID统计
    ...
    {
        $form = new Application_Form_InHospital();
        $extra = array();

        if ($this->getRequest()->isPost()) {
            if ($form->isValid($_POST)) {
                $newid = $this->modelInHospital->insertByForm($form, $extra);
    ...
    public function editAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($_POST)) {
                $result = $this->modelInHospital->updateByForm($form, $id);
                if ($result) {
        ...
        $oldData = $this->modelInHospital->getSingle(array('id'=>$id));
    ...
    public function deleteAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            try {
                $id = $this->_request->getParam('id');
                $this->modelInHospital->deleteData($id);
